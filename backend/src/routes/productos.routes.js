...
router.get("/:id", async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `SELECT 
        p.id_producto, p.nombre_producto, p.marca, p.descripcion, 
        p.precio_unitario, p.material, p.peso, p.imagen, 
        p.stock, p.id_categoria,
        c.nombre_categoria
      FROM productos p
...
router.put("/:id", upload.single("imagen"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      // No se sube nueva imagen → conservar existente
